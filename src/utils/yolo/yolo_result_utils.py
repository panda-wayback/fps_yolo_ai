...
import math
from ultralytics.engine.results import Boxes, Results  # YOLO 的返回类型
...
def select_best_target(result: Results) -> Optional[Tuple[Tuple[float, float], Tuple[float, float, float, float], float, int]]:
    """
    选择离图片中心最近的目标，返回中心点、边界框、置信度、类别ID
    ...
    Returns:
        ...
        - selected_target_point: (cx, cy)
        - selected_target_bbox: (x1, y1, x2, y2)
        - selected_target_confidence: float
        - selected_target_class_id: int
        如果没有检测结果，返回 None
    """
    boxes = result.boxes
    if boxes is None or len(boxes) == 0:
