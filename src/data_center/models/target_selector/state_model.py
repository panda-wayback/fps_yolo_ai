...
class TargetSelectorState(BaseState):
    ...
    # 目标信息
    selected_point: ReactiveVar[Tuple[float, float]] = None  # 选中目标的中心点 (x, y)
    selected_bbox: ReactiveVar[Tuple[float, float, float, float]] = None  # 选中目标的边界框 (x1, y1, x2, y2)
    selected_confidence: ReactiveVar[float] = None  # 选中目标的置信度
    selected_class_id: ReactiveVar[int] = None  # 选中目标的类别ID
