...
class ControllerModel(BaseState):
    ...
    sample_time: ReactiveVar[float] = 0.02  # 采样时间（秒）
    b0: ReactiveVar[float] = 0.95 # 控制增益
    w_cl: ReactiveVar[float] = 15.0  # 控制器带宽 (rad/s)
    k_eso: ReactiveVar[float] = 1.5  # ESO（扩张状态观测器）增益
    output_limits: ReactiveVar[tuple[float, float]] = (-780, 780)  # 输出限幅 (min, max)
